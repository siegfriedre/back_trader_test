...
新增独立的上市前研究层，用于互联网泡沫与 2008 年危机的假设性压力测试；不把模拟当作真实基金历史。

研究入口：`python prepare_research.py --end 2026-08-01`。完整方法、来源、限制和读取方式见 [RESEARCH_DATA.md](RESEARCH_DATA.md)。

当前仅完成新增模型的离线测试，尚未产出经过完整在线校验的新行情批次。

## 真实观测数据流程（get_data.py）
